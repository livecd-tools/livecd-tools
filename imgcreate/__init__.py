...
def main():
    ...
    ks = LiveCDParser(ksversion)
    try:
        ks.readKickstart(options.kscfg)
    ...
    target = LiveImageCreator(ks, fs_label)
    target.skip_compression = options.skip_compression
    target.isobase = options.base_on
    target.tmpdir = options.tmpdir

    try:
        target.mountImage(options.cachedir)
        target.installPackages()
        target.configureImage()
