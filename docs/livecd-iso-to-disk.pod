...
livecd-iso-to-disk - Loads bootable Live images onto USB/SD storage devices.
...
Disables the image validation process that occurs before the image is loaded from the original Live CD .iso image.  When this option is specified, the image is not verified before loading onto the target storage device.
...
Sets the Master Boot Record (MBR) of the target storage device to the mbr.bin file from the loading host system's syslinux directory.  This may be helpful in recovering a damaged or corrupted device.
...
Used when loading multiple images, to signal configuration of boot files for the image in the --livedir <dir> parameter.

=item --livedir <dir>

Used when multiple LiveOS images are loaded on a device to designate the directory <dir> for the particular image.

=item --compress   (default state for the original root filesystem)

The default, compressed SquashFS filesystem image is copied on image loading.  (This option has no effect if the source filesystem is already expanded.)

=item --skipcompress   (default option when  --xo is specified)

Expands the source SquashFS.img on image loading into the read-only /LiveOS/ext3fs.img root filesystem image file.  This avoids the system overhead of decompression during use at the expense of storage space.
