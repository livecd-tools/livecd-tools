...
usage() {
    ...
    livecd-iso-to-disk loads a Live CD/DVD/USB image (LiveOS) onto a USB/SD
    storage device (or any storage partition that will boot with a SYSLINUX
    bootloader).  The target storage device can then boot the loaded operating
    system on systems that support booting via the USB or the SD interface.
    The script requires a LiveOS source image and a target storage device.  The
    source image may be either a LiveOS .iso file, the currently-running LiveOS
    image, the device node reference for an attached device loaded with a LiveOS
    image, or a file backed by a block device loaded with a LiveOS image.  If
    the operating system supports persistent overlays for saving system changes,
    a pre-sized overlay may be included with the loaded image.

    Unless you request the --format option, loading an image does not destroy
    ...
    specify a fixed-size, persistent on disk overlay to hold changes to the root
    filesystem.  The build-time size of the root filesystem will limit the
    ...
    --unencrypted-home loading option.)  This filesystem is mounted on the /home
    ...
    --noverify
        ...
        loaded from the original Live CD .iso image.  When this option is
    ...
    --reset-mbr
        ...
        mbr.bin file from the loading host system's syslinux directory.  This
    ...
    --skipcopy
        ...
        configuration files on a previously loaded LiveOS device.)

    --force
        This option allows the loading script to bypass a delete confirmation
        dialog in the event that a pre-existing LiveOS directory is found on
        the target device.
    ...
    --multi
        Used when loading multiple images, to signal configuration of boot
        files for the image in the --livedir <dir> parameter.

    --livedir <dir>
        Used when multiple LiveOS images are loaded on a device to designate
    ...
    --compress   (default state for the original root filesystem)
        ...
        image loading.  (This option has no effect if the source filesystem is
        already expanded.)

    --skipcompress   (default option when  --xo is specified)
        Expands the source SquashFS.img on image loading into the read-only
